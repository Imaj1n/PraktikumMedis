...
# def manual_grid(uploaded_file):

#     st.title("📸 Ukur Piksel Manual")

#     # Inisialisasi session state untuk statistik ROI
...
#                 # Tombol Reset Tabel Statistik dipindah ke bawah
#                 if st.button("🔄 Reset Tabel Statistik"):
#                     st.session_state.statistics_table = pd.DataFrame(columns=["ROI", "Area", "Mean", "Min", "Max"])
#                     st.success("Tabel statistik telah di-reset.")

#                 # Tombol Simpan ROI
#                 if st.button("💾 Simpan ROI"):
#                     # cropped_image.save("roi_hasil.jpg")
#                     st.success("ROI berhasil disimpan sebagai roi_hasil.jpg")
...
#                 # Pilih kanal berdasarkan input
#                 if channel == 'red':
#                     channel_img = img_array[:, :, 0]  # Red dari RGB (Pillow)
#                 elif channel == 'green':
#                     channel_img = img_array[:, :, 1]
#                 elif channel == 'blue':
#                     channel_img = img_array[:, :, 2]
#                 else:
#                     return st.error("Channel tidak dikenali. Gunakan: 'red', 'green', atau 'blue'.")

#                 # Hitung statistik
#                 area = channel_img.size
#                 mean_val = channel_img.mean()
...
#             # Tombol Simpan
#             if st.button("💾 Simpan ROI"):
#                 st.success("ROI berhasil disimpan (simulasi).")
